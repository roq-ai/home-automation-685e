...
import { useHomeDeviceFindFirst, useRoqClient } from 'lib/roq';

import { HomeDeviceInterface } from 'interfaces/home-device';

function HomeDeviceViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const roqClient = useRoqClient();
  const queryParams = useMemo<{ include: { home: boolean; device: boolean; service: boolean } }>(
    () =>
  ...
  const { data, error, isLoading } = useHomeDeviceFindFirst(queryParams, {}, { disabled: !id });
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('home_device', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/home-devices/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                ...
                <FormListItem
                  ...
                  text={data?.warranty_expiry ? format(data?.warranty_expiry, 'dd-MM-yyyy') : ''}
                />
                ...
                {hasAccess('home', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Home"
                    text={
                      <Link as={NextLink} href={`/homes/view/${data?.home?.id}`}>
                        {data?.home?.address + ''}
                      </Link>
                    }
                  />
