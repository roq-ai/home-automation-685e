...
import { useRoqClient, useHomeDeviceFindFirst } from 'lib/roq';
...
function HomeDeviceEditPage() {
  ...
  const { data, error, isLoading, mutate } = useHomeDeviceFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: HomeDeviceInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.home_device.update({
        data: values as RoqTypes.home_device,
        where: {
          id,
        },
      });
