...
import { useServiceCount, useHomeCount, useDeviceCount } from 'lib/roq';

import { HomeListPage as ShowCaseList } from 'pages/homes';
...
function DashboardPage() {
  ...
  const homeStat = useHomeCount();

  const deviceStat = useDeviceCount();
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Homes',
      result: homeStat?.data,
      isLoading: homeStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Devices',
      result: deviceStat?.data,
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`homes`} passHref legacyBehavior>
                  <Button
